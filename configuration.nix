...
let
  user      = "admin";
  password  = "password";
  hostname  = "6928faa4a69fd0ee6da2a91a";
  homeDir   = "/home/${user}";
...
in
{
  ...
  boot = {
    ...
    loader.raspberryPi = {
      # keep whatever you already had here, plus:
      firmwareConfig = ''
        # Enable SPI buses
        dtparam=spi=on

        # Enable SPI1 with 3 chip-selects (needed for CAN1 on spi1-0)
        dtoverlay=spi1-3cs

        # Waveshare 2-CH CAN FD HAT (Mode A):
  ...
  hardware.enableRedistributableFirmware = true;boot.loader.raspberryPi = {
  # keep whatever you already had here, plus:
  firmwareConfig = ''
    # Enable SPI buses
    dtparam=spi=on
